class Museum:
    ...
    def get_info_exhibit(self, indx):
        founded_exhibit = self.exhibits[indx]
        return f'Описание экспоната {founded_exhibit.name}: {founded_exhibit.descr}'

    def __setattr__(self, key, value):
        if key == 'name' and type(value) == str or type(value) == list:
            object.__setattr__(self, key, value)
        else:
            raise TypeError('Неверное значение. Тип данных должен быть строкой (str)!')
